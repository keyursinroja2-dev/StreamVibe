...
export default function VideoPlayer({ src, thumbnail, autoplay = false, onEnded }: VideoPlayerProps) {
  ...
  const [seekFeedback, setSeekFeedback] = useState<{ dir: "back" | "fwd" | null; secs: number }>({ dir: null, secs: 0 });
  ...
  useEffect(() => {
    ...
    setDuration(0);
    setBuffered(0);
    // Native <video> with preload=metadata for fast start
  ...
  // Controls auto-hide (only when playing)
  ...
  function showSeekAnim(dir: "back" | "fwd", secs: number) {
    setSeekFeedback({ dir, secs });
    if (seekFeedbackTimer.current) clearTimeout(seekFeedbackTimer.current);
    seekFeedbackTimer.current = setTimeout(() => setSeekFeedback({ dir: null, secs: 0 }), 700);
  }
  ...
  function seekBy(secs: number) {
    ...
    showSeekAnim(secs > 0 ? "fwd" : "back", Math.abs(secs));
    showControlsNow();
  }
  ...
  function handleSeekBarMouseDown(e: React.MouseEvent<HTMLDivElement>) {
    ...
    const onMouseMove = (ev: MouseEvent) => {
      ...
      seekToPosition(ev.clientX, rect);
    };
    const onMouseUp = () => {
  ...
  // Handle taps on the video area — single tap shows controls, double tap seeks
  function handleVideoTap(e: React.MouseEvent<HTMLDivElement> | React.TouchEvent<HTMLDivElement>) {
    ...
    if (now - lastTapRef.current.time < doubleTapThreshold) {
      // Double tap — seek
      const isLeft = clientX < containerW * 0.4;
      const isRight = clientX > containerW * 0.6;
      if (isLeft) seekBy(-10);
      else if (isRight) seekBy(10);
  ...
  if (!src) {
    return (
      <div className="relative aspect-video bg-black rounded-xl overflow-hidden flex items-center justify-center">
        {thumbnail && <img src={thumbnail} alt="" className="absolute inset-0 w-full h-full object-cover opacity-40" />}
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "relative bg-black select-none group outline-none",
    ...
    >
      ...
      {seekFeedback.dir === "back" && (
        <div className="absolute left-[10%] top-1/2 -translate-y-1/2 flex flex-col items-center gap-1 pointer-events-none animate-fade-in">
          <div className="bg-black/70 rounded-full p-3 backdrop-blur-sm">
            <RotateCcw className="w-8 h-8 text-white" />
          </div>
          <span className="text-white text-sm font-semibold bg-black/60 px-3 py-0.5 rounded-full">-{seekFeedback.secs}s</span>
        </div>
      )}
      {seekFeedback.dir === "fwd" && (
        <div className="absolute right-[10%] top-1/2 -translate-y-1/2 flex flex-col items-center gap-1 pointer-events-none animate-fade-in">
          <div className="bg-black/70 rounded-full p-3 backdrop-blur-sm">
      ...
      >
        <button
          ...
          className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-black/60 flex items-center justify-center pointer-events-auto transition-all duration-150 hover:bg-black/80 hover:scale-110 active:scale-95 backdrop-blur-sm"
        >
          {playing
          ...
          <div className="flex items-center gap-1 sm:gap-2 mt-1">
            ...
            <div className="relative">
              ...
              {showSettings && (
                ...
                >
                  <p className="text-white/50 text-[11px] mb-2 font-semibold uppercase tracking-wide">Playback Speed</p>
                  {SPEEDS.map((r) => (
                    <button
                      key={r}
                      data-controls
                      ...
                      className={cn(
                        ...
                        playbackRate === r ? "text-primary font-semibold bg-primary/10" : "text-white hover:bg-white/10"
                      )}
                    >
                      {r === 1 ? "Normal (1×)" : `${r}×`}
                    </button>
